...
namespace netCollections.Generic
{
    ...
    public class Bag<T> : CollectionBase<T>, ICloneable
    {
        ...
        public Bag<T> Clone()
        {
            Bag<T> newBag = new(equalityComparer, keyEqualityComparer, hash.Clone(null), count);
            return newBag;
        }
        ...
        public Bag<T> CloneContents()
        {
            ...
            Hash<KeyValuePair<T, int>> newHash = new(equalityComparer);
            ...
            foreach (KeyValuePair<T, int> pair in hash)
            {
                ...
                newHash.Insert(newPair, true, out _);
            }
        ...
        {
            return hash.Find(NewPair(item), false, out _);
        }
        ...
        private void AddSecure(T item)
        {
            ...
            if (!hash.Insert(pair, false, out KeyValuePair<T, int> existing))
            {
                ...
                hash.Insert(newPair, true, out _);
            }
            ++count;
        }
