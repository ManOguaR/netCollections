...
namespace netCollections.Generic
{
    ...
    public class Set<T> : CollectionBase<T>, ICollection<T>, ICloneable
    {
        ...
        public Set<T> Clone()
        {
            Set<T> newSet = new(equalityComparer, hash.Clone(null));
            return newSet;
        }
        ...
        public Set<T> CloneContents()
        {
            ...
            Set<T> clone = new(equalityComparer);
        ...
        {
            return hash.Find(item, false, out _);
        }
        ...
        {
            return !hash.Insert(item, true, out _);
        }
        ...
        public void IntersectionWith(Set<T> otherSet)
        {
            ...
            Hash<T> newHash = new(equalityComparer);
        ...
        public void SymmetricDifferenceWith(Set<T> otherSet)
        {
            ...
            if (otherSet.Count > Count)
            {
                ...
                foreach (T item in this)
                {
                    if (newHash.Find(item, false, out _))
                        newHash.Delete(item, out _);
                    else
                        newHash.Insert(item, true, out _);
                }
                hash = newHash;
