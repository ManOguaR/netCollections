...
namespace System.Internals
{
    ...
    internal class RedBlackTree<T> : IEnumerable<T>
    {
        ...
        {
            if (findFirst)
                return FirstItemInRange(EqualRangeTester(key), out _);
            else
                return LastItemInRange(EqualRangeTester(key), out _);
        }
        ...
        public bool Insert(T item, DuplicatePolicy dupPolicy, out T previous)
        {
            ...
            while (node != null)
            {
                ...
                {
                    node = InsertSplit(ggparent, gparent, parent, node, out bool rotated);
            ...
            InsertSplit(ggparent, gparent, parent, node, out _);
        ...
        public int DeleteRange(RangeTester rangeTester)
        {
            ...
            do
            {
                deleted = DeleteItemFromRange(rangeTester, true, out _);
                if (deleted)
                    ++counter;
            } while (deleted);
