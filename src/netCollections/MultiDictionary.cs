...
namespace netCollections
{
    ...
    public class MultiDictionary<TKey, TValue> : MultiDictionaryBase<TKey, TValue>, ICloneable
    {
        ...
        public sealed override void Add(TKey key, TValue value)
        {
            ...
            if (hash.Find(keyValues, false, out MultiDictionary<TKey, TValue>.KeyAndValues existing))
            {
        ...
        public sealed override bool Contains(TKey key, TValue value)
        {
            ...
            if (hash.Find(find, false, out MultiDictionary<TKey, TValue>.KeyAndValues item))
            {
        ...
        public MultiDictionary<TKey, TValue> CloneContents()
        {
            ...
            MultiDictionary<TKey, TValue> newDict = new(allowDuplicateValues, keyEqualityComparer, valueEqualityComparer);
                ...
                else
                {
                    for (int i = 0; i < item.Count; ++i)
                    {
                        if (item.Values[i] == null)
                            valuesClone[i] = default;   // Really null, because we know TKey isn't a value type.
                        else
